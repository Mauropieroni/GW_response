...
class TestUtils(unittest.TestCase):
    def test_pixels(self):
        ...
        self.assertAlmostEqual(float(jnp.sum(jnp.abs(theta - save_arr))), 0.0)
        save_arr = np.load(TEST_DATA_PATH + "pixel_phi.npy")
        self.assertAlmostEqual(float(jnp.sum(jnp.abs(phi - save_arr))), 0.0)
        angular_map = pixel_class.angular_map
        save_arr = np.load(TEST_DATA_PATH + "pixel_angular_map.npy")
        self.assertAlmostEqual(float(jnp.sum(jnp.abs(angular_map - save_arr))), 0.0)
    ...
    def test_arm_length_exp_lisa(self):
        lisa = gwr.LISA()
