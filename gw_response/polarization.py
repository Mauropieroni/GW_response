...
@jax.jit
def unit_vec(theta: jax.Array, phi: jax.Array) -> jax.Array:
    """
    Computes the unit vector pointing from the SSB towards each requested sky position
    (``\\hat{n}``, not the propagation direction ``\\hat{k} = -\\hat{n}``), this is the
    sky-position unit vector in Hartwig, Lilley, Muratore & Pieroni (arXiv:2303.15929)
    Sec. II A (e.g., eq. 2.8'; that paper leaves the sign implicit).
...
@jax.jit
def uv_analytical(theta: jax.Array, phi: jax.Array) -> tuple[jax.Array, jax.Array]:
    """
    ...
    orthonormal triad,
        ...
        ``\\hat{u} x \\hat{v} = unit_vec(theta, phi)``

    to construct the GW polarization basis. Matches the LISA LDC Manual
    (LISA-LCST-SGS-MAN-001) Sec 6.1.2 antiparallel convention

        ``(\\hat{u}, \\hat{v}, \\hat{k})`` with ``k = -unit_vec(theta, phi)``

    also evaluating at the antipodal sky position ``(pi - theta, phi + pi)`` reproduces
    the LDC convention. This is also equivalent to the right-handed construction in
    Eq. 2.9 of Hartwig et al. (arXiv:2303.15929),

        ``\\hat{u} = (\\hat{k} x \\hat{e}_z)/|\\hat{k} x \\hat{e}_z|``
...
def polarization_vectors(u: jax.Array, v: jax.Array) -> tuple[jax.Array, jax.Array]:
    ...
    Returns:
        ...
        vectorial_index (3)), corresponding to ``(u - i v)`` and ``(u + i v)``.
    """
    # The output will be pixels, vectorial index
    return (u - 1j * v), (u + 1j * v)


@jax.jit
def polarization_vectors_angles(
    theta: jax.Array, phi: jax.Array
...
def polarization_tensors_LR(u: jax.Array, v: jax.Array) -> tuple[jax.Array, jax.Array]:
    """
    Computes the left/right circular gravitational wave polarization tensors as in
    eq. 2.10 of Hartwig, Lilley, Muratore & Pieroni (arXiv:2303.15929):

        ``e^{L/R}_{ab} = e^+_{ab} ∓ i*e^x_{ab}``

    exactly, with no residual normalization difference. Built directly from
...
def polarization_tensors_and_wavevector(
    polarization: str, theta: jax.Array, phi: jax.Array
) -> tuple[jax.Array, jax.Array, jax.Array]:
    """
