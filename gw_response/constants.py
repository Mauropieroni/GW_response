...
class PhysicalConstants:
    ...
    This class provides a convenient way to access commonly used physical constants,
    ensuring consistency and clarity across different parts of the code. The constants
    are set as class attributes with predefined values.

    Attributes:
        light_speed (float): The speed of light in meters per second (m/s). Default is
            299792458.0.
        hour (float): The duration of an hour in seconds. Default is 3600 seconds.
        day (float): The duration of a day in seconds, calculated as 24 hours. Default
            is 86400 seconds.
        yr (float): The duration of a year in seconds, accounting for leap years.
            Default is 365.25 days.
        sidereal_day (float): Earth's rotation period relative to the fixed stars (as
            opposed to the Sun), in seconds. Default is 86164.0905 seconds.
        Hubble_over_h (float): Hubble constant divided by the dimensionless Hubble
            parameter 'h'. Units are 1/second (1/s).

    The class is frozen using chex.dataclass, meaning its instances are immutable and
    ...
    sidereal_day: float = (
        86164.0905  # Earth's rotation period relative to the fixed stars, in s
    )
...
class BasisTransformations:
    ...
    which is crucial for accurate representation and analysis in astronomical models.
    """
