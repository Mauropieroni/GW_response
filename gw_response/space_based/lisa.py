...
class LISA(Detector):
    ...
    Attributes:
        ps (PhysicalConstants): Physical constants used in the
            response/noise computations.
        fmin (float): The minimum frequency sensitivity for LISA, set to
            3.0e-5 Hz.
        fmax (float): The maximum frequency sensitivity for LISA, set to
            5.0e-1 Hz.
        armlength (float): The length of LISA's arm, set to 2.5e9 meters.
        deg (float): The angular displacement of LISA behind the Earth, set
            to 20 degrees.
        res (float): The expected resolution of LISA, set to 1e-6.
        orbit_approximant (str): The orbit model to use: 'rigid' (a perfectly
            rigid, non-flexing constellation), 'keplerian' (each satellite on
            its own heliocentric Keplerian ellipse, following Martens &
            Joffre 2021, arXiv:2101.03040), or 'numeric' (interpolated from
            precomputed orbit data). Default is 'rigid'.
        orbit_file (str or None): Path to a file with numerical orbit data
            (see `load_numerical_orbits`). Required when
            `orbit_approximant` is 'numeric', ignored otherwise.
        orbit_interpolation_method (str): The interpolation method used to
            ...
            'catmull-rom', 'monotonic', 'monotonic-0', or 'akima'. Default is
            'linear'. Only used when `orbit_approximant` is 'numeric'.
        keplerian_tilt_parameter (float): Dimensionless inclination parameter
            delta_1 used by the 'keplerian' orbit approximant. Default is
            5/8, which minimizes arm length flexing. Ignored otherwise.
        ...
        response (Response): Response object used to compute LISA's response
    ...
    def _vertex_positions(self, time_in_years: ArrayLike) -> jax.Array:
        ...
        Args:
            time_in_years (ArrayLike): The time at which the positions are to
                be calculated, in years.
        ...
        Returns:
            jax.Array: The positions of LISA satellites as calculated by the
                ...
                `self.orbit_interpolator`/`self.keplerian_*` attributes as
    ...
    def _detector_arms(self, time_in_years: ArrayLike) -> jax.Array:
        ...
        Args:
            time_in_years (ArrayLike): The time at which the arm matrix is to
                be computed, in years.

        Returns:
            jax.Array: The arm matrix of the LISA detector as calculated by
                the :func:`gw_response.space_based.orbits.LISA_arms_matrix`
                function, using `self.orbit_approximant` (and the related
                `self.orbit_interpolator`/`self.keplerian_*` attributes as
                required by that model).
        """
    ...
    def combination_matrix(
        ...
        Returns:
            jax.Array: The TDI projection matrix for the requested
                combination.
        """
    ...
    def linear_response_from_single_link(
        ...
        Args:
            ...
            combination_matrix (ArrayLike): TDI projection matrix as built
                by :meth:`combination_matrix`.
    ...
    def quadratic_response_from_single_link(
        ...
        """
        Computes the sky-resolved quadratic TDI response as the
        cross-spectrum of the linear response with its own conjugate, summed
        over polarizations and Hermitian conjugation.

        Args:
            linear_integrand (dict): Linear TDI response per polarization,
                as returned by :meth:`linear_response_from_single_link`.

        Returns:
            dict: The sky-resolved quadratic TDI response per doubled
                polarization letter (e.g. "LL", "RR").
        """
        quadratic_integrand = {}
    ...
    def integrate_quadratic_response(
        ...
        :func:`gw_response.response.quadratic_response_integrated`.

        Args:
            quadratic_integrand (dict): Sky-resolved quadratic TDI response
                per doubled polarization letter, as returned by
                :meth:`quadratic_response_from_single_link`.

        Returns:
            dict: The sky-averaged quadratic TDI response per doubled
                polarization letter.
        """
        return {
    ...
    def single_link_noise(
        ...
        **noise_parameters,
        ...
        """
        Computes the total (test-mass + OMS) single-link noise covariance
        matrix. See
        :func:`gw_response.space_based.noise.single_link_TM_acceleration_noise_variance`
        and
        :func:`gw_response.space_based.noise.single_link_OMS_noise_variance`.

        Args:
            frequency_array (ArrayLike): Frequency values, in Hz, at which to
                evaluate the noise.
            arms_matrix_rescaled (ArrayLike): Detector arm vectors rescaled
                by the arm length.
            x_vector (ArrayLike): Vector of ``2 pi f L / c`` values over
                frequency.
            **noise_parameters: Must contain ``TM_acceleration_parameters``
                and ``OMS_parameters`` (each a per-arm noise amplitude
                vector of length 6).

        Returns:
            jax.Array: The single-link test-mass + OMS noise covariance
                matrix.
        """
    ...
    def project_noise(
        ...
        Args:
            combination_matrix (ArrayLike): TDI projection matrix as built
                by :meth:`combination_matrix`.
            single_link_noise (jax.Array): Single-link noise covariance as
