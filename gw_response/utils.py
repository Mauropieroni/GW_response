...
@jax.jit
def combine_single_link(combination_matrix, single_link):
    """
    Applies a detector's channel-combination matrix to per-link responses.

    combination_matrix: (..., x_vector, channels, arms)
    single_link: (..., x_vector, arms, pixels)
    returns: (..., x_vector, channels, pixels)
    """
    return jnp.einsum("...ijk,...ikl->...ijl", combination_matrix, single_link)


@jax.jit
def project_noise_matrix(combination_matrix, single_link_noise):
    """
    ...
    combination_matrix: (..., x_vector, channels, arms)
    single_link_noise: (..., x_vector, arms, arms)
    returns: (..., x_vector, channels, channels)
    """
...
@jax.jit
def arms_matrix_from_vertex_positions(m1, m2, m3):
    """
