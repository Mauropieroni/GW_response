...
from gw_response.single_link_utils import (
    ...
    armlength_suppression_function,
)
...
def geometrical_factor_PC_angles_static(
    arms_matrix_rescaled: jax.Array, theta: jax.Array, phi: jax.Array
    ...
    Args:
        ...
        theta (float or jax.Array): Colatitude(s) of the sky position(s), in radians.
        phi (float or jax.Array): Longitude(s) of the sky position(s), in radians.
...
def xi_k_no_G_static(
    ...
    the geometrical antenna-pattern factor is applied. This is basically everything in
    eq. 2.13 of Hartwig, Lilley, Muratore & Pieroni (arXiv:2303.15929), i.e.,

        ``e^{-2 π i f \\hat{k} · \\vec{L}_{ij}} M_{ij}(f,\\hat{k})``

    except for the geometrical factor ``G^A(\\hat{k},\\hat{l}_{ij})``.
    ...
    return armlength_suppression_function(comb_plus, comb_minus, x_vector)
...
def xi_k_A_static(
    ...
    the light-travel-time and position phase factors. This matches eq. 2.13 of Hartwig,
    Lilley, Muratore & Pieroni (arXiv:2303.15929):

        ``ξ_{ij}^A(f,\\hat{k}) = e^{-2 π i f \\hat{k} · \\vec{L}_{ij}} \times
                                \times M_{ij}(f,\\hat{k}) G^A(\\hat{k},\\hat{l}_{ij})``
...
def xi_k_A_PC_angles_static(
    ...
    Args:
        arms_matrix_rescaled (jax.Array): Detector arm vectors rescaled by the arm
            length, with shape (configurations, vectorial_index (3), arms (6)).
        theta (jax.Array): Polar angle(s) of the sky position, with shape (pixels,).
        phi (jax.Array): Azimuthal angle(s) of the sky position, with shape (pixels,).
        x_vector (jax.Array): Vector of ``2 pi f L / c`` values over frequency.
...
def single_link_response_static(
    ...
    """
    Computes the full single-link strain response, combining the response kernel with
    the light-travel-time delay and the satellite position phase factors. The full
    frequency-domain single-link transfer function in eq. 2.12 of Hartwig, Lilley,
    Muratore & Pieroni (arXiv:2303.15929) including the prefactor
    ``i (f/f_{ij}) e^{2 π i f (t-L_{ij})}`` and the position phase term
    ``e^{-2 π i f \\hat{k} · \\vec{x}_i}`` combined with ``ξ_{ij}^A``.
    ...
    # Note: in eq. 2.12 of arXiv:2303.15929, the prefactor is just ``i``, here we
    # include a minus sign (``-1j``) to convert from time-delay to fractional frequency.
    # This is to match the LDC convention as well as Numpy and JAX's FFT conventions.
